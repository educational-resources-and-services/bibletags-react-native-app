...
import { getVersionInfo, getOriginalVersionInfo, safelyExecuteSelects, equalObjs, orderedStatusesArray, getNewPromiseWithExternalResolve } from "../utils/toolbox"

const cachedTranslationsOfWordsInMyVersionsByKey = {}
const promisesByKey = {}
...
const useTranslationsOfWordInMyVersions = ({
  ...
  const translationsOfWordsInMyVersions = useMemoAsync(
    async () => {
      ...
      const key = `${originalLoc} ${downloadedVersionIds.join(',')}`

      if(!cachedTranslationsOfWordsInMyVersionsByKey[key] && promisesByKey[key]) {

        const promiseWithExternalResolve = getNewPromiseWithExternalResolve()
        promisesByKey[key].push(promiseWithExternalResolve)
        await promiseWithExternalResolve

      } else if(!cachedTranslationsOfWordsInMyVersionsByKey[key]) {
        promisesByKey[key] = []
        ...
        cachedTranslationsOfWordsInMyVersionsByKey[key] = translationsOfWordsInMyVersions
        promisesByKey[key].forEach(promiseWithExternalResolve => promiseWithExternalResolve.resolve())
      ...
      return cachedTranslationsOfWordsInMyVersionsByKey[key]

    },
    [ originalLoc, downloadedVersionIds ],
    {},
  )
